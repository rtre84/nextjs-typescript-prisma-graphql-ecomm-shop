...
function OrderRowList(props) {
    ...
    return (<ul>
      {rows.map(({ id, quantity, product: { name, price, brand, thumbnail } }) => (<article key={id} itemProp="itemListElement" itemScope itemType="http://schema.org/Product">
            <div className="image">
            ...
            <div className="quantity">
              <UpdateOrderRowMutation {...orderRowMutationProps} variables={{ id, quantity: quantity - 1 }}>
                <input type="submit" value="-" aria-label={`Remove 1 ${name} from cart`}/>
              </UpdateOrderRowMutation>

              <UpdateOrderRowMutation {...orderRowMutationProps} variables={{ id, quantity }}>
                {({ updateOrderRowMutation }) => (<OrderRowQuantityInput quantity={quantity} onChange={newQuantity => {
        updateOrderRowMutation({
            id,
            ...
            <div className="delete">
              <UpdateOrderRowMutation {...orderRowMutationProps} variables={{ id, quantity: 0 }}>
...
//# sourceMappingURL=OrderRowList.jsx.map